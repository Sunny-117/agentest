/**
 * playground/advanced/run.ts
 *
 * 高阶示例入口：数据驱动（cases.json）+ 脚本驱动（tests/*.test.ts）混合运行。
 *
 * 运行：
 *   npx tsgo playground/advanced/run.ts
 */
...
import type { JsonCase } from '../../src/types.js';

const __dirname = fileURLToPath(new URL('.', import.meta.url));

async function main(): Promise<void> {
  // ── 1. 自动发现并加载 tests/*.test.ts ────────────────────────────────────
  ...
  // ── 2. 合并数据驱动 + 脚本驱动用例 ──────────────────────────────────────
  const casesRaw: JsonCase[] = JSON.parse(
